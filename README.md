...
Codificación FEC: core/ts36212_channel_coding.py - COMPONENTE CENTRAL
Controlador de Simulación: controller/simulation_mgr.py - Orquestación del FEC
Interfaz Principal: ui/main_window.py
Núcleo OFDM: core/ofdm_ops.py
Modelo de Canal: core/channel.py
Configuración LTE: core/config.py
Forward Error Correction (FEC) - Pipeline Completo
Fundamento Teórico
...
Latencia: Retransmisiones aumentan el delay (crítico en VoIP, streaming)
Eficiencia espectral: Corregir errores localmente libera recursos de radio
Confiabilidad: Garantiza calidad en condiciones de canal adversas
Teoría de Shannon
...
La ganancia de codificación 
G
c
G 
c
​
  es la reducción en SNR necesaria para alcanzar el mismo BER con FEC:

G_c (dB) = SNR_sin_FEC - SNR_con_FEC  |_mismo_BER
Para código convolucional K=7, R=1/3: 
G
c
≈
3
...
c
​
 ≈3−6 dB (según el esquema de modulación)
...
Ubicación en código: controller/simulation_mgr.py
...
Ubicación: core/ts36212_channel_coding.py
...
Archivo: core/ts36212_channel_coding.py
...
Inicialización: Registro de 24 bits a cero

reg = 0
mask = (1 << 24) - 1  # 0xFFFFFF
Procesamiento de bits del mensaje:
...
Procesamiento de 24 ceros adicionales (finalización):
...
Validación Adicional en el Simulador (simulation_mgr.py):
...
Añadir tail bits si se termina:

if terminate:
    in_bits = np.concatenate([bits, np.zeros(6, dtype=np.uint8)])
else:
    in_bits = bits
Procesar cada bit de entrada:
...
Blanquea el espectro: Evita concentración de energía en frecuencias específicas
Elimina patrones repetitivos: Secuencias de muchos 0s o 1s → distribución uniforme
Sincronización: Secuencia conocida facilita detección de inicio de trama
En LTE real, se usan secuencias Gold (combinación de dos m-sequences).

Implementación: apply_scrambling()
Archivo: core/utils.py (función específica)
...
Método controlador: controller/simulation_mgr.py - calculate_ber_curve()

Proceso
Cargar imagen de prueba (mismos bits para todas las pruebas)
Iterar sobre SNR: De 0 a 30 dB en 10 puntos
Para cada modulación:
Transmitir los mismos datos
Calcular BER = errores / bits_totales
Generar dos gráficas separadas:
...
PAPR = max|x(t)|² / E[|x(t)|²]
PAPR_dB = 10·log₁₀(PAPR)
Ubicación: controller/simulation_mgr.py - calculate_papr_distribution()

¿Por qué es crítico en OFDM?
Amplificadores no lineales: PAPR alto → saturación/clipping → distorsión
Eficiencia energética: PA debe operar con back-off → desperdicio de potencia
Cumplimiento normativo: Clipping genera regrowth espectral (viola máscaras FCC)
...
Protección robusta contra errores mediante CRC-24A + Código Convolucional K=7, R=1/3
Decodificación eficiente con algoritmo de Viterbi optimizado
Análisis cuantitativo de ganancia de codificación (3-6 dB)
Validación experimental comparando BER con/sin FEC
